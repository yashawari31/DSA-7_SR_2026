...
dll *delete_at_pos(dll *head,int pos);
void *delete_at_end(dll *head);
...
int main()
{
   ...
   start=insert_at_pos(start,4);
   display(start);

   printf("\ndata after deleting a head node:\n");
   start=delete_at_head(start);
   display(start);
   start=delete_at_pos(start,2);
   printf("Data after deleting node at position 2\n");
   display(start);

   printf("Data after deleting Last node\n");
   delete_at_end(start);
   display(start);
